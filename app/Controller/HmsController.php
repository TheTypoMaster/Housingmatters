...
$this->loadmodel('user');
$conditions=array('society_id'=>$s_society_id,'deactive'=>1);
$result=$this->user->find('all',array('conditions'=>$conditions));
...
//*//smscode//*//$payload = file_get_contents('http://alerts.sinfini.com/api/web2sms.php?workingkey='.$working_key.'&sender='.$sms_sender.'&to='.$mobile.'&message='.$sms1.'');
$to=$email;
...
$s_date_ex0.$s_date_ex1.$s_date_ex2.$time_h.$time_m;
 //*//smscode//*//$payload = file_get_contents('http://alerts.sinfini.com/api/web2sms.php?workingkey='.$working_key.'&sender='.$sms_sender.'&to='.$mobile_im.'&message='.$massage_str.'&time='.$s_date_ex0.$s_date_ex1.$s_date_ex2.$time_h.$time_m);
...
function hms_sms_ip()
{
	...
	foreach($result as $data)
	{
		...
		return $sms=(object)array("working_key"=>$w,"sms_sender"=>$s);
...
<img src='$ip".$this->webroot."/as/hm/hm-logo.png'/><span  style='float:right; margin:2.2%;'>
<span class='test' style='margin-left:5px;'><a href='https://www.facebook.com/HousingMatters.co.in' target='_blank' ><img src='$ip".$this->webroot."/as/hm/fb.png'/></a></span>
<a href='#' target='_blank'><img src='".$this->webroot."/as/hm/tw.png'/></a><a href'#'><img src='$ip".$this->webroot."/as/hm/ln.png'/ class='test' style='margin-left:5px;'></a></span>
...
<p style='font-size:15px;'>$reply</p>
...
foreach($myArray as $child){
	...
	if(empty($child[1])){
		$output = json_encode(array('type'=>'error', 'text' => 'Please enter a age'));
        die($output);
	}
...
$output = json_encode(array('type'=>'add_tr', 'text' => 'Your family details have been updated.'));
				die($output);

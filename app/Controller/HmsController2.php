...
$remening_days=floor($datediff/(60*60*24));
	...
	$alert_result_count=$this->alert->find('count',array('conditions'=>$conditions));
		...
		if($alert_result_count==0)
		{
		$this->send_alert('<span class="label" style="background-color:#6d1b81;" ><i class="icon-question-sign"></i></span>','Voting for Poll - <b>'.$question.'</b> will close on <b>'.$close_date.'</b>',7,$poll_id,'Polls',$visible_user_id);
		}
	...
	$alert_result_count=$this->alert->find('count',array('conditions'=>$conditions));
		...
		if($alert_result_count==0)
		{
		$this->send_alert('<span class="label" style="background-color:#6d1b81;" ><i class="icon-question-sign"></i></span>','Voting for Poll - <b>'.$question.'</b> have been closed on <b>'.$close_date.'</b>',7,$poll_id,'closed_polls',$visible_user_id);
		}
...
$incomplete=100-$progres;
	...
	$alert_result_count=$this->alert->find('count',array('conditions'=>$conditions));
		...
		if($alert_result_count==0)
		{
		$this->send_alert('<span class="label label-success"><i class="icon-user"></i></span>','Your Profile is <b>'.$incomplete.'%</b> incomplete',101,$s_user_id,'profile',array($s_user_id));
		}
...
{
$this->loadmodel('role_privilege');
$conditions=array("society_id" => $s_society_id ,"role_id" => $role_id,"sub_module_id" => $sub_module_id,"module_id" => $module_id);
...
$per_type = (int)$collection['penalty']['tax_type'];
}
...
else { $ok=1; $error_msg[]="Flat should not be empty in row ".$row_no.".";	break;}
